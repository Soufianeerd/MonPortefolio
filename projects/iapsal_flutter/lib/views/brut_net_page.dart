...
class BrutNetPage extends StatefulWidget {
  const BrutNetPage({super.key});

  @override
  State<BrutNetPage> createState() => _BrutNetPageState();
}

class _BrutNetPageState extends State<BrutNetPage> {
  int _iAnnee = 2026;
  int _iMoisCalcul = 1;
  int _iclsImpots = 100;
  int _iclsMutu = 1;
  ...
  void _calculer() {
    ...
    if (_brutController.text.isEmpty) {
      _montrerInfo(l10n.infoReqGross);
      return;
    }
    ...
    loSalaire.dBrut =
        double.tryParse(_brutController.text.replaceAll(',', '.')) ?? 0;
    ...
    bool lResult = CalculSalaire().calculBrutNet(loSalaire);
    ...
    if (lResult) {
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) =>
              ResultatPage(salaire: loSalaire, titre: l10n.titleBrutNet),
        ),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(l10n.titleBrutNet)),
